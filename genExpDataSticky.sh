...
# tagescl, 4K BTB in runExp.sh, 20% (10?) chance of data cache missing for PT
...
for app in postgres $advancement_apps; do
    ...
    # Eliminating switch fragmentation
    ~/scarab_out/runExp.sh $app $uoc_capacity "_NOSWITCHFRAG" "--uoc_ic_switch_frag_disable=1" &
done

for list_type in resteer ; do #shared
    for app in deepsjeng $advancement_apps; do
        for stickiness in 90 98 99; do
            for num_priority_pws in 50 100 1000; do #100 1000 2000 3000
                ...
                ~/scarab_out/runExp.sh $app $uoc_capacity "_STICKY${stickiness}_${num_priority_pws}PINNED_${list_type}_PLUS_PWS_UNTIL_BSTALL_NOSWITCHFRAG" "--prioritize_pws_after_first_sticky_until_backend_stall=1 $baseline_fdip_on --uop_cache_repl=12 --priority_line_stickiness_percent=$stickiness --num_priority_pws=$num_priority_pws --pw_priority_list_filepath=$pw_prio_list_file --uoc_ic_switch_frag_disable=1" &
            done
            sleep 1
